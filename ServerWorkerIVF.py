...
from VideoStreamIVF import VideoStream
...
class ServerWorker:
	...
	# Divide a frame into RTP Packets
	def divide_into_RTPpackets(self, frame, frame_no, mss=1300):
		frame_len = len(frame)
		pkt_idx = 0
		...
		while pkt_idx < frame_len:
			if pkt_idx + mss > frame_len:
				frame_slice = frame[pkt_idx:] #frame_slice and frame are bytearray
			else:
				frame_slice = frame[pkt_idx:pkt_idx + mss]

			# address = self.clientInfo['rtspSocket'][1]   #!!!! this is a tuple object ("address" , "")
			self.clientInfo['rtpSocket'].sendto(self.makeRtp(frame_slice, frame_no),(self.clientInfo['rtspSocket'][1][0], port))
			pkt_idx += mss
	...
	def sendRtp(self):
		...
		while True:
			...
			if data:
				...
				try:
					self.divide_into_RTPpackets(data,frameNumber)
					time.sleep(jit)
